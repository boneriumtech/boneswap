...
/* 
 ...
 Bonerium Tech Layer 2 is a high-performance solution designed to provide fast and reliable transactions to Ethereum users. 
 The solution is built on top of the Ethereum network using a unique layer 2 scaling solution that allows for faster and more cost-effective transactions. 
 
 $BONES is the official token for the Bonerium network, and it plays an important role in the governance of the network. 
 Through $BONES, users have the ability to vote on important proposals and decisions that directly impact the functionality of the network. 

 https://t.me/boneriumnetwork
...
contract BONES is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    constructor(
        ...
        } else if(block.chainid == 9117){
            _dexRouter = 0x81D4257F7110C73C351F777f484Be1869165E6aB; // Bonerium Chain
        } else {
    ...
    function updateLimits(uint256 _maxBuy, uint256 _maxWallet) external onlyOwner {
        require(_maxBuy >= 10 && _maxWallet >= 10, "Error");
        
        maxTransactionAmount = totalSupply() * _maxBuy / 1000;
